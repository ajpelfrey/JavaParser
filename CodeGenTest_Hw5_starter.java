...
class CodeGenTest_Hw5_starter {
	...
	void hw5_9() throws Exception {
		...
		assertEquals(Color.YELLOW.getRGB(), p);
	}
...
// Obtain AST from parser
		edu.ufl.cise.cop4020fa23.ast.AST ast =
				ComponentFactory.makeParser(source).parse();
...
// Generate Java code
		String javaCode = (String)
				ast.visit(ComponentFactory.makeCodeGenerator(), packageName);
...
// #region Tests by Christina Castillo
	...
	void doRunsTwice() throws Exception {
		String source = """
                ...
                :>
                """;
		int gcd = (Integer) PLCLangExec.runCode(packageName, source,
				10, 15);
		assertEquals(5, gcd);
	}
	...
	void doRunsOnce() throws Exception {
		String source = """
                ...
                :>
                """;
		int result = (Integer) PLCLangExec.runCode(packageName,
				source, 0);
	...
	void widthImplicitLoop() throws Exception {
		...
		BufferedImage output = (BufferedImage)
				PLCLangExec.runCode(packageName, source, url, w, h);
	...
	// I currently can’t get this testcase to pass, but I wrote what I
	...
	void extractRedFromImage() throws Exception {
		...
		BufferedImage result = (BufferedImage)
				PLCLangExec.runCode(packageName, source, w, h);
		BufferedImage expected = ImageOps.makeImage(w, h);
		expected = ImageOps.setAllPixels(expected, PixelOps.pack(50,
				0, 0));
		compareImages(expected, result);
	}

	// Although a function to do binary operations between images and
	...
	void binaryImagePixelOp() throws Exception {
		...
		TypeCheckException e = assertThrows(TypeCheckException.class,
				() -> PLCLangExec.runCode(packageName, source, url));
	...
	void binaryPackedPixelScalarOp() throws Exception {
		String source = """
                ...
                """;
		Object result = PLCLangExec.runCode(packageName, source);
		int expected = ImageOps.binaryPackedPixelScalarOp(ImageOps.OP.TIMES,
				Color.RED.getRGB(), 2);
